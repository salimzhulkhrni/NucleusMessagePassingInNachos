...
Thread::Thread(char* threadName)
{
    ...
    //id = kernel->threadIdentiferCounter;
    //kernel->threadIdentiferCounter++;

    string thread_name = name;

    //cout<<"name is: "<<thread_name <<endl;
   // kernel->thread_tableMap[name] = this;

    kernel->thread_tableMap[thread_name] = this;

    //cout<<"thread name: "<<thread_name<<" from map: thread's address: "<<kernel->thread_tableMap[thread_name]<<endl;

    isWaitingForMessage = 0;  // initially not waiting for messgae
    isWaitingForAnswer = 0;  // initially not waiting for answer

    //buff = new ThreadMessageBuffer();
    //threadBufferPool = new List<ThreadMessageBuffer*>(); // buffer 
    //buff->setSenderID('a');
    //cout<<"val: "<<buff->getSenderID()<<endl;
