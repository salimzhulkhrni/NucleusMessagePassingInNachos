...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        cout<<"User PRGM NAME: "<<userProgName<<endl;
        uProgName.push_back(userProgName);
	    i++;
	}
    ...
    if(uProgName.size() > 0){
        ...
        for(int i=0;i<uProgName.size();i++){

            //cout<<"name is: "<<uProgName[i]<<endl;
            char *progName = uProgName[i];

            int size=0;

            char *name;

            for(int i=8;progName[i] != '\0';i++){
            ...
            for(int i=8,j=0;progName[i] != '\0';i++,j++){
            ...
            //cout<<"prog name is:"<<name<<endl;
            

            Thread *t = new Thread(name);
            t->Fork((VoidFunctionPtr) RunUserProg, (void *) uProgName[i]);
