...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        ASSERT(i + 1 < argc);   // next argument is int
        messageLimit = atoi(argv[i + 1]);
        cout<<"Message Limit : "<<messageLimit<<endl;
        i++;
        ...
        cout<<"IN KERNEL: QUANTUM: "<<this->quantum<<endl;
        //cout<<"q:"<<quantum<<endl;
     }
    }
    cout<<"Message Limit : "<<messageLimit<<endl;
...
Kernel::Initialize()
{
    ...
    for(int i=0;i<10;i++){
            //allocate memory for 10 buffers
        if(bufferPool[i] == NULL){

            bufferPool[i] = new ThreadMessageBuffer();
            bufferPool[i]->setBufferID(i); 
            //cout<<"itr: "<<i<<"val: "<<bufferPool[i]->getBufferID()<<endl;
    ...
    alarm = new Alarm(randomSlice,this->quantum);   // start up time slicing
